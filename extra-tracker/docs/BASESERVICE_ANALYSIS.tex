...
\begin{itemize}
    \item \textbf{Regex Injection Fix}: Aggiunto metodo \texttt{\_escapeRegex()} per sanitizzare input in \texttt{search()}
    \item \textbf{Error Handling Standardizzato}: \texttt{\_getUserId()} ora usa \texttt{AppError.unauthorized()} invece di \texttt{Error} generico (ritorna 401 invece di 500)
    \item \textbf{Validazione ObjectId}: \texttt{findById()} valida ObjectId prima della query MongoDB
    \item \textbf{Validazione Limit/Skip (DoS Prevention)}: \texttt{find()} e \texttt{paginate()} validano e normalizzano parametri (limit: 1-100, skip: min 0) - \textbf{Previene DoS con limit=1000000}
\end{itemize}
\end{importantbox}
...
\begin{importantbox}{✅ Correzioni Applicate}
Le seguenti vulnerabilità sono state \textbf{risolte} nel codice:
\begin{itemize}
    \item ✅ \textbf{Regex Injection}: \texttt{search()} ora escapa caratteri speciali
    \item ✅ \textbf{Error Handling}: \texttt{\_getUserId()} usa \texttt{AppError} standardizzato
    \item ✅ \textbf{Validazione ObjectId}: \texttt{findById()} valida ID prima della query
    \item ✅ \textbf{Validazione Limit/Skip}: \texttt{find()} e \texttt{paginate()} validano e normalizzano parametri
\end{itemize}
\end{importantbox}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \textbf{Stato attuale}: ✅ \textbf{RISOLTO} - \texttt{\_getUserId()} lancia \texttt{AppError.unauthorized()} con messaggio user-friendly
        \item \textbf{Implementazione}: Errore standardizzato con codice \texttt{INVALID\_TENANT\_SCOPE} e suggerimento
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{Miglioramento Futuro}: Considerare optimistic locking con \texttt{versionKey} di Mongoose per casi critici
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{Miglioramento Futuro}: Validare \texttt{populateFields} nel constructor
    \end{itemize}
    
    \item \textbf{search() con regex injection}:
    \begin{itemize}
        \item \textbf{Stato attuale}: ✅ \textbf{RISOLTO} - \texttt{search()} usa \texttt{\_escapeRegex()} per sanitizzare input
        \item \textbf{Implementazione}: Metodo privato \texttt{\_escapeRegex()} escapa tutti i caratteri speciali regex
        \item \textbf{Protezione}: Previene attacchi come \texttt{searchTerm = ".*"} che scaricherebbero tutto il DB
    \end{itemize}
    
    \item \textbf{Limit/Skip senza validazione}:
    \begin{itemize}
        \item \textbf{Stato attuale}: ✅ \textbf{RISOLTO} - \texttt{find()} e \texttt{paginate()} validano e normalizzano parametri
        \item \textbf{Implementazione}: 
        \begin{itemize}
            \item \texttt{limit}: min 1, max 100 (default 20) - \textbf{Previene DoS}
            \item \texttt{skip}: min 0 (previene valori negativi)
            \item \texttt{page}: min 1 (in \texttt{paginate()})
        \end{itemize}
        \item \textbf{Protezione}: 
        \begin{itemize}
            \item Previene DoS con \texttt{limit: 999999} che bloccherebbe il server
            \item Impedisce a un client malevolo di richiedere milioni di documenti
            \item Usa \texttt{safeLimit} e \texttt{safeSkip} per chiarezza del codice
        \end{itemize}
    \end{itemize}
    
    \item \textbf{findById() senza validazione ObjectId}:
    \begin{itemize}
        \item \textbf{Stato attuale}: ✅ \textbf{RISOLTO} - \texttt{findById()} valida ObjectId prima della query
        \item \textbf{Implementazione}: Usa \texttt{mongoose.Types.ObjectId.isValid()} per validazione
        \item \textbf{Protezione}: Previene errori MongoDB e attacchi con ID malformati
    \end{itemize}
\end{enumerate}
    ...
    if (!tenantScope || !tenantScope.tenantId) {
        throw AppError.unauthorized(
            ...
            { 
                ...
                suggestion: 'Assicurati che il middleware tenantContext sia applicato.'
            }
        );
    }
    return tenantScope.tenantId;
...
async search(tenantScope, searchTerm, additionalFilters = {}) {
    ...
    \begin{lstlisting}
// ✅ IMPLEMENTATO - Previene DoS con limit=1000000
async find(tenantScope, filters = {}, options = {}) {
    // ...
...
// ✅ IMPLEMENTATO - Previene DoS in paginazione
async paginate(tenantScope, filters = {}, { page = 1, limit = 20 } = {}) {
    ...
    \item \textbf{Validazione ObjectId in findById()}:
    \begin{lstlisting}
// ✅ IMPLEMENTATO
async findById(tenantScope, id, options = {}) {
    if (!id || !mongoose.Types.ObjectId.isValid(id)) {
...
\begin{itemize}
    ...
    \item ✅ \textbf{Correzioni Applicate}: Regex escaping, validazione input, error handling standardizzato
\end{itemize}

\textbf{Correzioni Implementate}:
\begin{enumerate}
    \item ✅ \textbf{Regex Injection Fix}: Metodo \texttt{\_escapeRegex()} previene attacchi regex injection
    \item ✅ \textbf{Error Handling}: \texttt{\_getUserId()} usa \texttt{AppError.unauthorized()} standardizzato
    \item ✅ \textbf{Validazione ObjectId}: \texttt{findById()} valida ID prima della query
    \item ✅ \textbf{Validazione Limit/Skip}: \texttt{find()} e \texttt{paginate()} validano e normalizzano parametri
\end{enumerate}
...
\begin{enumerate}
    ...
    \item Implementare optimistic locking per update() in casi critici
    \item Validare \texttt{populateFields} nel constructor
\end{enumerate}
