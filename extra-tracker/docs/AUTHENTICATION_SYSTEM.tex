...
\begin{lstlisting}[caption=Refresh Token con Validazione Multi-Sessione]
async refreshAccessToken(refreshToken, deviceInfo = {}) {
    // 1. Verifica firma JWT
    const payload = this.verifyToken(refreshToken, 'refresh');

    // 2. Trova utente con array refreshTokens
    ...
    // 4. Trova sessione specifica nell'array
    ...
    // 5. Aggiorna lastUsedAt per questa sessione
    session.lastUsedAt = new Date();

    // 6. Genera nuovi token (rotation) - mantieni stesso device info
    ...
    // 7. Rimuovi vecchia sessione e aggiungi nuova (rotation)
...
\textbf{Meccanismo di sicurezza con multi-sessione:}
\begin{enumerate}
    \item Verifica firma JWT (garantisce autenticità)
    \item Cerca sessione specifica nell'array \code{refreshTokens}
    \item Se sessione non trovata: possibile furto, invalida tutte le sessioni
