...
\subsubsection{Errore CORS: "Not allowed by CORS"}

\textbf{Problema:} Il frontend viene bloccato dal backend con errore CORS.

\textbf{Cause Comuni:}
\begin{enumerate}
    \item \code{FRONTEND\_URL} non configurato nel file \code{.env}
    \item Porta frontend diversa da quella nella whitelist CORS
    \item Browser usa \code{127.0.0.1} invece di \code{localhost}
    \item Frontend su rete locale (IP 192.168.x.x) non permesso
\end{enumerate}

\textbf{Soluzione:}

\begin{enumerate}
    \item \textbf{Verifica file \code{.env}:}
    \begin{lstlisting}
# server/.env
FRONTEND_URL=http://localhost:5173
    \end{lstlisting}
    
    \item \textbf{Verifica porta frontend:}
    \begin{itemize}
        \item Vite default: \code{5173}
        \item React default: \code{3000}
        \item Controlla console frontend per porta effettiva
    \end{itemize}
    
    \item \textbf{Verifica configurazione CORS:}
    \begin{lstlisting}[caption=server/config/security.js]
cors: {
    origin: (origin, callback) => {
        ...
        // In sviluppo, permetti qualsiasi localhost
        if (!isProduction) {
    ...
    \item \textbf{Debug CORS:}
    \begin{itemize}
        \item Controlla console backend per log CORS
        \item Verifica header \code{Origin} nella richiesta (DevTools → Network)
        \item Verifica che \code{credentials: true} sia configurato
    \end{itemize}
    
    \item \textbf{Test Rapido:}
    \begin{lstlisting}
# Verifica che FRONTEND_URL sia caricato
cd server
node -e "require('dotenv').config(); console.log(process.env.FRONTEND_URL)"
    \end{lstlisting}
\end{enumerate}

\textbf{Configurazione Completa CORS:}

La configurazione CORS include:
\begin{itemize}
    \item \textbf{Origini permesse}: \code{localhost}, \code{127.0.0.1}, varianti porte
    \item \textbf{In sviluppo}: Permette qualsiasi localhost per flessibilità
    \item \textbf{Credentials}: \code{true} per inviare cookies
    \item \textbf{Headers}: Content-Type, Authorization, etc.
    \item \textbf{Metodi}: GET, POST, PUT, DELETE, PATCH, OPTIONS
\end{itemize}
